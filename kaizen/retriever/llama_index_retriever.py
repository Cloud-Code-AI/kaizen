...
class RepositoryAnalyzer:
    def __init__(self, database_config):
        ...
        self.index = None
        self.engine = create_engine(f"postgresql://{os.environ['PG_USER']}:{os.environ['PG_PASSWORD']}@{os.environ['PG_HOST']}:{os.environ['PG_PORT']}/{os.environ['db_name']}")
    ...
    def store_function_in_db(self, function_name, file_path, start_line, end_line):
        with self.engine.connect() as connection:
            ...
            file_query = text("""
                ...
                RETURNING file_id
            """)
            file_result = connection.execute(file_query, {
            ...
            """)
            snippet_result = connection.execute(snippet_query, {
                'file_id': file_id,
            ...
            node_query = text("""
                ...
                RETURNING node_id
            """)
            node_result = connection.execute(node_query, {
                'file_id': file_id,
