...
class UnitTestGenerator:
    ...
    def generate_tests(self, file_path: str, content: str = None, max_critique: int = 3, output_path: str = None, verbose: bool = False):
    ...
    def _get_parser(self, file_extension):
        ...
        parser_module = importlib.import_module(f"kaizen.parsers.{parser_class_name.lower()}")
    ...
    def _process_item(self, item, file_extension, file_path, folder_path):
        ...
        test_code, usage = self.generate_ai_tests(item, item["source"], file_extension)
    ...
    def _prepare_test_file_path(self, item, file_extension, folder_path):
        ...
        self.log_step("Prepare file name and path", f"Test file path: {test_file_path}")
        return test_file_path

    def generate_ai_tests(self, item, source_code, file_extension):
        prompt_template = self.LANGUAGE_PROMPTS.get(file_extension, UNIT_TEST_PLAN_PROMPT)
        ...
        response, usage = self._generate_actual_tests(item, source_code, test_scenarios, prompt_template)
        self.total_usage = self.provider.update_usage(self.total_usage, usage)
    ...
    def _generate_actual_tests(self, item, source_code, test_scenarios, prompt_template):
    ...
    def format_test_scenarios(self, scenarios):
        formatted_scenarios = ""
        for category in ["normal_cases", "edge_cases", "error_handling", "boundary_conditions"]:
